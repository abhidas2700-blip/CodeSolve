...
export default function UserAuditPage() {
  ...
  const loadCurrentUser = () => {
    try {
      const savedUser = localStorage.getItem('qa-current-user');
      if (savedUser) {
  ...
  const submitSkippedSample = () => {
    ...
    // Use the current user information from localStorage for the auditor
    const auditorId = currentUser?.id || null;
    const auditorName = currentUser?.username || "Unknown Auditor";
    
    console.log("Submitting skipped sample with auditor:", auditorName, "ID:", auditorId);
    ...
    const skippedSample = {
      ...
      agent: agentName,
      agentId: agentId,
      auditor: auditorId, // Use the actual current user ID
      auditorName: auditorName, // Use the actual current username
                ...
                <Card>
                                    ...
                                    >
                                      ...
                                      <SelectContent>
                                        {(() => {
                                          ...
                                          let options = question.options?.split(',').map(o => o.trim()) || [];
                  ...
                  <CardFooter className="flex justify-between">
                    ...
                    <div className="flex space-x-2">
                      ...
                      {/* Show Skip Sample button for all users with audit right, not just admin/manager/teamleader */}
